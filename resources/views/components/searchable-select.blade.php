...
<div wire:key="{{ rand() }}">
    <div class="w-full" x-init="getInitData" x-data="{
        isOpen: false,
        title: '---',
        search: null,
        ...
        <button @click="handleButtonClick"
            {{ $attributes->merge(['type' => 'button', 'class' => ' h-[35px] inline-flex items-center justify-between w-full px-2 py-2 text-center border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none']) }}>
            <span x-text="title" class=" truncate"></span>
            <template x-if="selectedOption">
                <x-svgs.close @click="selectOption(null)" class=" shrink-0 w-4 h-4 z-10" />
            </template>
        </button>

        <div x-cloak x-ref="dropDownBox" x-show="isOpen" @click.away="isOpen = false"
            class=" {{ $position }} fixed mt-1 border bg-white border-gray-200 dark:border-gray-700 dark:bg-gray-900 rounded-md shadow-md">
