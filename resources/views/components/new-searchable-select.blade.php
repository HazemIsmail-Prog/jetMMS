...
{{-- @php
    if (isset($attributes->get('x-model'))) {
...
<div
    ...
    x-data="{
        ...
        const selectedOptions = Array.from(selectElement.options).filter(option => this.currentlySelected.includes(option.value));
        selectedOptions.forEach(option => option.selected = true);
        
        // Dispatch change event to trigger Livewire update
        this.$nextTick(() => selectElement.dispatchEvent(new Event('change'))); --}}
                    this.$refs.select.dispatchEvent(new Event('change'));
                } else {
...
>
    ...
    <select class="hidden" x-ref="select" x-model="{{ $attributes->get('x-model') }}" {{ $multipule ? 'multiple' : ''}}>
        <option value=""></option>
