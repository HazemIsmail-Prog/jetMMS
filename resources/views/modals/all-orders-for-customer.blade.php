<div 
    x-data="allOrdersForCustomer"
    x-on:open-all-orders-for-customer-modal.window="openModal"
>
    <!-- Modal -->
    <div x-on:close.stop="hideModal" x-on:keydown.escape.window="dismissible ? hideModal() : null" x-show="showModal"
        ...
        <div x-show="showModal"
            ...
            <template x-if="selectedCustomer">
                <div class="p-4"> 
                    <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-4" x-text="'{{ __('messages.all_orders_for_customer') }} ' + selectedCustomer.name"></h2>
                            ...
                            <tbody>
                                <template x-for="order in orders" :key="order.id">
                                    <x-tr>
                                        ...
                                        <x-td>
                                            <div x-text="order.completed_at ? order.formatted_completion_date : order.cancelled_at ? order.formatted_cancellation_date : '-'"></div>
                                            <div x-show="order.completed_at || order.cancelled_at" class="text-[0.7rem]" x-text="order.completed_at ? order.formatted_completion_time : order.cancelled_at ? order.formatted_cancellation_time : '-'"></div>
                                            <!-- reason if cancelled using x-show -->
                                            <div class="text-[0.7rem] text-red-500" x-show="order.cancelled_at" x-text="order.cancellation_reason"></div>
                                        </x-td>
    ...
    <script>
        function allOrdersForCustomer() {
            return {
                ...
                getOrders() {
                    this.loading = true;
                    axios.get(`/customers/${this.selectedCustomer.id}/getAllOrders`)
                        .then(response => {
