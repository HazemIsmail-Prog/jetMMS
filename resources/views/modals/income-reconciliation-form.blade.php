<div 
    x-data="incomeReconciliationFormModal"
    x-on:open-income-reconciliation-form-modal.window="openModal"
>
    <!-- Modal -->
    <div x-on:close.stop="hideModal" x-on:keydown.escape.window="dismissible ? hideModal() : null" x-show="showModal"
        ...
        <div x-show="showModal"
            ...
            <template x-if="selectedIncomeInvoice">
                <div class="p-4"> 
                    <div class="mb-4">
                        <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ __('messages.add_reconciliation') }}</h2>
                        <p class="text-sm text-gray-600 dark:text-gray-400" x-text="'Invoice #' + selectedIncomeInvoice.id"></p>
                    </div>
                        ...
                        <div class=" space-y-3">
                            ...
                            <div>
                                ...
                                <x-select class="w-full" required x-model="form.type" id="type">
                                    ...
                                    <option value="sales_refund">{{ __('messages.sales_refund') }}</option>
    ...
    <script>
        function incomeReconciliationFormModal() {
            return {
                dismissible: true,
                ...
                async openModal(e) {
                    this.resetForm();
                    this.showModal = true;
                    this.selectedIncomeInvoice = e.detail.selectedIncomeInvoice;
                },
                ...
                get calculatedRemainingAmount() {
                    ...
                    const totalPayments = this.selectedIncomeInvoice.payments?.reduce((acc, payment) => acc + parseFloat(payment.amount), 0);
                    const totalReconciliations = this.selectedIncomeInvoice.income_reconciliations?.reduce((acc, reconciliation) => acc + parseFloat(reconciliation.amount), 0);
                    const remainingAmount = parseFloat(this.selectedIncomeInvoice.amount) - (totalPayments || 0) - (totalReconciliations || 0);
                    return Number((remainingAmount).toFixed(3));
                },
                ...
                save() {
                    ...
                    // console.log(this.selectedIncomeInvoice.id);
                    // return;

                    axios.post(`/income-invoices/${this.selectedIncomeInvoice.id}/reconciliations`, this.form)
                        .then(response => {
                            this.$dispatch('income-invoice-updated',{
                                incomeInvoice: response.data.data,
                            });
                            this.hideModal();
                        })
                        .catch(error => {
                            console.error('Error creating income reconciliation:', error);
                            alert(error.response?.data?.error || 'An error occurred while creating the income reconciliation');
                        })
                        .finally(() => {
                            this.loading = false;
                        });
