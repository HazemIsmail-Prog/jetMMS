            ...
            <tbody>
                @foreach ($this->statuses as $status)
                    <x-tr>
                        ...
                        <x-td class=" !whitespace-normal">{{ @$status->reason ?? '-' }}</x-td>
                        <x-td class=" !whitespace-normal">{{ @$status->technician->name ?? '-' }}</x-td>
                        <x-td>
                            <div>{{ $status->created_at->format('d-m-Y') }}</div>
                            <div class=" text-xs">{{ $status->created_at->format('H:i') }}</div>
                        </x-td>
                        <x-td class=" !whitespace-normal">{{ @$status->creator->name }}</x-td>
                    </x-tr>
                @endforeach
            </tbody>
        </x-table>
