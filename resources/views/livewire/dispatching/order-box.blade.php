    ...
    <div class="mt-2">
        ...
        <div
            ...
            <select wire:model.live="change_order_technician.{{ $order->id }}.technician_id"
                {{ in_array($order->status_id, [3, 7]) ? 'disabled' : '' }}
    ...
    <div class="flex items-center justify-between w-full mt-3 text-xs font-medium text-gray-400">
        ...
        <div class=" flex gap-2 items-center">
            ...
            <div wire:click="$dispatch('showCommentsModal',{order_id:{{ $order->id }}})"
                ...
                <span class="text-xs">{{ $order->all_comments > 0 ? $order->all_comments : '' }}</span>
            </div>

            <div wire:click="$dispatch('showInvoicesModal',{order_id:{{ $order->id }}})"
                class="flex items-center gap-1">
                <x-svgs.invoice class="h-4 w-4" />
                <span class="text-xs">{{ $order->invoices_count > 0 ? $order->invoices_count : '' }}</span>
            </div>
        ...
        <div class=" flex gap-2 items-center">
            @if ($order->status_id != 5)
                <div wire:confirm="{{ __('messages.are_u_sure') }}" wire:click="holdOrder({{ $order->id }})"
                    class="flex items-center">
                    <x-svgs.clock class="h-4 w-4" />
                </div>
            @endif
            <div wire:confirm="{{ __('messages.are_u_sure') }}" wire:click="cancelOrder({{ $order->id }})"
                class="flex items-center">
