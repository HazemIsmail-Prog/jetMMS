                    ...
                    <div class=" grid grid-cols-3 items-center gap-2">
                        <div class="flex flex-col flex-1">
                            <x-input required class="p-1" type="text"
                                placeholder="{{ __('messages.description_ar') }}" wire:model="attachment.description_ar"
                                id="description_ar" dir="rtl" />
                            <x-input-error for="attachment.description_ar" />
                        </div>
                        <div class="flex flex-col flex-1">
                            <x-input required class="p-1" type="text"
                                placeholder="{{ __('messages.description_en') }}" wire:model="attachment.description_en"
                        ...
                        <div class="flex flex-col flex-1">
                            <label for="attachment.file" @class([
                                ...
                                <div class="flex flex-col items-center justify-center p-1 ">
                                    <p 
                                    @class([
                                        'text-sm text-gray-500 dark:text-gray-400 font-semibold' => !$attachment['file'],
                                        'text-sm text-green-500 dark:text-green-400 font-semibold' => $attachment['file'],
                                    ])>
            ...
            @if ($currentRecord)
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th scope="col" class="px-6 py-1 whitespace-nowrap text-start">{{ __('messages.description') }}</th>
                            <th scope="col" class="px-6 py-1 whitespace-nowrap text-center">{{ __('messages.expirationDate') }}</th>
                            ...
                            <tr
                                ...
                                <td class="px-6 py-1 text-end whitespace-nowrap">
                                    <div class=" flex items-center justify-end gap-2">
                                        ...
                                        <x-svgs.trash wire:confirm="{{ __('messages.are_u_sure') }}" wire:click="delete({{ $attachment }})"
                                            class="w-6 h-6" />
                                    </div>
                                </td>
                            </tr>
