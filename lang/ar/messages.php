<?php

return [

    'You are logged in!' => 'تم تسجيل الدخول بنجاح',
    'Sign In to your account' => 'تسجيل الدخول لحسابك',
    'dashboard' => 'لوحة المعلومات',
    'new' => 'جديد',
    'settings' => 'الاعدادات',
    'users' => 'المستخدمين',
    'departments' => 'الأقسام',
    'reports' => 'التقارير',
    'report1' => 'التقرير الأول',
    'report2' => 'التقرير الثاني',
    'report3' => 'التقرير الثالث',
    'operations' => 'العمليات',
    'customers' => 'العملاء',
    'orders' => 'الطلبات',
    'name' => 'الاسم',
    'status' => 'الحالة',
    'active' => 'مفعل',
    'inactive' => 'معطل',
    'save' => 'حفظ',
    'name_required' => 'يرجى ادخال الاسم',
    'added_successfully' => 'تمت الإضافة بنجاح',
    'updated_successfully' => 'تم التعديل بنجاح',
    'deleted_successfully' => 'تم الحذف بنجاح',
    'department' => 'القسم',
    'actions' => 'العمليات',
    'update' => 'تحديث',
    'back' => 'تراجع',
    'name_used' => 'هذا الاسم مستخدم من قبل',
    'delete_r_u_sure' => 'هل انت متأكد من الحذف ؟',
    'add_user' => 'مستخدم جديد',
    'add_department' => 'قسم جديد',
    'username' => 'اسم المستخدم',
    'email' => 'الايميل',
    'password' => 'كلمة المرور',
    'edit_department' => 'تعديل القسم',
    'username_required' => 'يرجى ادخال اسم المستخدم',
    'username_used' => 'اسم المستخدم مستخدم من قبل',
    'wrong_email' => 'الايميل غير صحيح',
    'password_required' => 'يرجى ادخال كلمة المرور',
    'password_min_5' => 'كلمة المرور 5 احرف او اكثر',
    'department_required' => 'يرجى اختيار القسم',
    'login' => 'تسجيل الدخول',
    'remember_me' => 'تذكرني',
    'title_required' => 'يرجى ادخال الوظيفة',
    'title' => 'الوظيفة',
    'add_customer' => 'عميل جديد',
    'mobile' => 'النقال',
    'phone' => 'الهاتف',
    'area' => 'المنطقة',
    'block' => 'القطعة',
    'street' => 'الشارع',
    'building' => 'المبنى',
    'floor' => 'الدور',
    'apartment' => 'الشقة',
    'notes' => 'ملاحظات',
    'cid' => 'الرقم المدني',
    'mobile_required' => 'يرجى ادخال النقال',
    'area_required' => 'يرجى ادخال المنطقة',
    'block_required' => 'يرجى ادخال القطعة',
    'tel' => 'التليفون',
    'address' => 'العنوان',
    'mobile_used' => 'الرقم مستخدم من قبل',
    'jadda' => 'جادة',
    'add_order' => 'طلب جديد',
    'assigned_to' => 'تعيين مراقب',
    'customer_data' => 'بيانات العميل',
    'order_details' => 'تفاصيل الطلب',
    'estimated_start_date' => 'تاريخ استحقاق الطلب',
    'customer' => 'العميل',
    'order_description' => 'وصف المشكلة',
    'customer_id_required' => 'يرجى اختيار العميل',
    'assigned_to_required' => 'يرجى تعيين مراقب',
    'estimated_start_date_required' => 'يرجى ادخال تاريخ الاستحقاق',
    'customer_id' => 'كود العميل',
    'add_customer_to_database' => 'اضافة العميل الى قاعدة البيانات',
    'supervisor_required' => 'يرجى اختيار المراقب',
    'created_at' => 'تاريخ الانشاء',
    'dist_panel' => 'شاشة التوزيع',
    'enter_comment' => 'اكتب تعليق',
    'date' => 'التاريخ',
    'technician' => 'الفني',
    'supervisor' => 'المراقب',
    'unassigned' => 'غير معين',
    'no_orders' => 'لا يوجد طلبات',
    'titles' => 'الوظائف',
    'add_title' => 'وظيفة جديدة',
    'emails' => 'البريد الالكتروني',
    'add_email' => 'اضافة بريد الكتروني',
    'type' => 'النوع',
    'free' => 'مجاني',
    'paid' => 'مدفوع',
    'type_required' => 'يرجى ادخال النوع',
    'email_required' => 'يرجى ادخال الايميل',
    'lines' => 'الخطوط',
    'devices' => 'الأجهزة',
    'user' => 'المستخدم',
    'serial_no' => 'الرقم المسلسل',
    'add_line' => 'اضافة سطر',
    'add address' => 'اضافة عنوان',
    'add phone' => 'اضافة هاتف',
    'number' => 'الرقم',
    'address_required' => 'يرجى اختيار العنوان',
    'phone_required' => 'يرجى اختيار الهاتف',
    'save_with_order' => 'حفظ وانشاء طلب',
    'Duplicate Order' => 'العميل لديه طلب :department قيد التنفيذ',
    'show_completed' => 'اظهار الطلبات المنفذة',
    'show_cancelled' => 'اظهار الطلبات الملغية',
    'search' => 'بحث',
    'cancel' => 'الغاء',
    'edit_title' => 'تعديل الوظيفة',
    'serial' => 'مسلسل',
    'order assigned cannot change department' => 'الطلب معين على فني - لا يمكن تغيير القسم',
    'service_type' => 'نوع الخدمة',
    'service_type_required' => 'يرجى اختيار نوع الخدمة',
    'statuses' => 'الحالات',
    'name_ar' => 'الاسم العربي',
    'name_en' => 'الاسم الانجليزي',
    'color' => 'اللون',
    'name_ar_required' => 'الاسم العربي مطلوب',
    'name_en_required' => 'الاسم الانجليزي مطلوب',
    'name_ar_used' => 'الاسم العربي مستخدم',
    'name_en_used' => 'الاسم الانجليزي مستخدم',
    'completed_date' => 'تاريخ التنفيذ',
    'customer_name' => 'اسم العميل',
    'customer_phone' => 'تليفون العميل',
    'creator' => 'مقدم الطلب',
    'completed_at' => 'تاريخ التنفيذ',
    'something went wrong' => 'حدث خطأ ما .. يرجى المحاولة لاحقا',
    'edit_customer' => 'تعديل بيانات العميل',
    'time' => 'الوقت',
    'order_progress' => 'حالات الطلب',
    'order_comments' => 'التعليقات',
    'comment' => 'التعليق',
    'view_order' => 'عرض الطلب',
    'no_records_found' => 'لا يوجد نتائج',
    'results_number' => 'عدد النتائج',
    'short_block' => 'ق',
    'short_street' => 'ش',
    'short_jadda' => 'ج',
    'short_building' => 'م',
    'lang' => 'اللغة',
    'send' => 'ارسال',
    'sender' => 'المرسل',
    'change_status' => 'تغيير الحالة',
    'reason' => 'السبب',
    'dispatching' => 'التوزيع',
    'show_today_orders_only' => 'عرض طلبات اليوم فقط',
    'show_previous_orders_only' => 'عرض الطلبات السابقة فقط',
    'show_all' => 'عرض جميع الطلبات',
    'roles' => 'الأدوار',
    'permissions' => 'الصلاحيات',
    'add_role' => 'دور جديد',
    'edit_user' => 'تعديل المستخدم',
    'section' => 'القسم',
    'role' => 'الدور',
    'order_number' => 'رقم الطلب',
    'accept' => 'قبول',
    'done' => 'تم',
    'is_service' => 'خدمة',
    'on_hold' => 'توقف مؤقت',
    'cancel_order' => 'الغاء الطلب',
    'todays_orders_only' => 'طلبات اليوم فقط',
    'todays_completed' => 'منفذ اليوم',
    'number_already_exist' => 'رقم الهاتف مستخدم من قبل',
    'artisan_commands' => 'الأوامر',
    'command' => 'الأمر',
    'run' => 'تنفيذ',
    'export_to_excel' => 'تصدير اكسيل',
    'cancelled_at' => 'تاريخ الالغاء',
    'areas' => 'المناطق',
    'add_area' => 'اضافة منطقة',
    'roles_required' => 'الدور مطلوب',
    'no_comments_found' => 'لا يوجد تعليقات',
    'arrived' => 'وصول',
    'all_areas' => 'المنطقة (الكل)',
    'view_tech_orders' => 'عرض طلبات الفني',
    'no_customers_found' => 'لم يتم العثور على نتائج',
    'edit_area' => 'تعديل منطقة',
    'edit_role' => 'تعديل دور',
    'hold_order_confirmation' => 'سيتم وضع الطلب في حالة التوقف المؤقت. هل انت متأكد؟',
    'cancel_order_confirmation' => 'سيتم الغاء الطلب. هل انت متأكد؟',
    'monthly_orders_statistics' => 'الاحصائية الشهرية للطلبات',
    'month' => 'الشهر',
    'year' => 'السنة',
    'Sunday' => 'الأحد',
    'Monday' => 'الاثنين',
    'Tuesday' => 'الثلاثاء',
    'Wednesday' => 'الأربعاء',
    'Thursday' => 'الخميس',
    'Friday' => 'الجمعة',
    'Saturday' => 'السبت',
    'total' => 'الاجمالي',
    'department_technician_statistics' => 'احصائية تنفيذ الاقسام والفنيين',
    'completed' => 'منفذ',
    'service' => 'الخدمة',
    'services' => 'الخدمات',
    'add_service' => 'اضافة خدمة',
    'edit_service' => 'تعديل خدمة',
    'min_price' => 'اقل سعر',
    'max_price' => 'اعلى سعر',
    'min_price_required' => 'يرجى ادخال اقل سعر',
    'max_price_required' => 'يرجى ادخال اعلى سعر',
    'invoice' => 'الفاتورة',
    'quantity' => 'الكمية',
    'unit_price' => 'السعر',
    'service_total' => 'اجمالي الخدمة',
    'create_invoice' => 'انشاء فاتورة',
    'invoice_created_successfully' => 'تم انشاء الفاتورة بنجاح',
    'invoices' => 'الفواتير',
    'invoice_number' => 'رقم الفاتورة',
    'amount' => 'المبلغ',
    'payment_status' => 'حالة الدفع',
    'paid_amount' => 'اجمالي المدفوع',
    'all' => 'الكل',
    'pending' => 'لم يتم الدفع',
    'partially_paid' => 'مدفوع جزئي',
    'paid' => 'مدفوع',
    'received_time' => 'وقت الاستلام',
    'arrived_time' => 'وقت الوصول',
    'completed_time' => 'وقت التنفيذ',
    'receiveToComplete' => 'من الاستلام الى التنفيذ',
    'arriveToComplete' => 'من الوصول الى التنفيذ',
    'technician_timing' => 'توقيتات تنفيذ الطلبات حسب الفني',
    'remaining_amount' => 'المتبقي',
    'no_invoices_found' => 'لا يوجد فواتير',
    'delete_invoice_confirmation' => 'سيتم حذف الفاتورة الحالية. هل انت متأكد؟',
    'payments' => 'المدفوعات',
    'no_payments_found' => 'لا يوجد مدفوعات',
    'create_payment' => 'انشاء دفعة',
    'payment_method' => 'طريقة الدفع',
    'cash' => 'كاش',
    'knet' => 'كي نت',
    'delete_payment_confirmation' => 'سيتم حذف الدفعة الحالية. هل انت متأكد؟',
    'new_order_created' => 'تم انشاء طلب جديد',
    'you_have_new_order_no' => 'لديك طلب جديد برقم',
    'order' => 'طلب',
    'change_order_status' => 'نغيير حالة طلب',
    'admin' => 'مسؤول النظام',
    'edit_order_status' => 'تعديل حالات طلب',
    'add_row' => 'اضافة صف',
    'roles_required' => 'يرجى اختيار الدور',
    'shifts' => 'الشفتات',
    'shift' => 'الشفت',
    'start_time' => 'وقت البداية',
    'end_time' => 'وقت النهاية',
    'add_shift' => 'اضافة شفت',
    'edit_shift' => 'تعديل شفت',
    'undefined_shift' => 'شفت غير محدد',
    'from' => 'من',
    'to' => 'الى',
    'part' => 'القطعة',
    'parts' => 'قطع الغيار',
    'customer_info' => 'بيانات العميل',
    'customer_invoice' => 'فاتورة العميل',
    'thanks_message' => 'شكرا لثقتكم بنا',
    'selected_services_required' => 'يجب اختيار بند واحد على الاقل',
    'clear_search_to_continue' => 'يحب مسح المكتوب في البحث للمتابعة',
    'no_services_selected' => 'لم يتم اضافة خدمات بعد للفاتورة',
    'no_parts_selected' => 'لم يتم اضافة قطع غيار بعد للفاتورة',
    'are_u_sure' => 'هل انت متأكد ؟',
    'confirm_saving' => 'نعم متأكد',
    'back_to_invoice' => 'لا , اكمل النواقص',
    'print_detailed_invoice' => 'فاتورة تفصيلية',
    'print_non_detailed_invoice' => 'فاتورة مختصرة',
    'print_invoice' => 'طباعة الفاتورة',
    'orderTag' => 'الوسم',
    'marketing_counter' => 'احصائية التسويق',
    'marketing' => 'تسويق',
    'marketings' => 'التسويق',
    'add_marketing' => 'اضافة تسويق',
    'information' => 'استفسار',
    'service_not_available' => 'خدمة غير متوفرة',
    'comments' => 'التعليقات',
    'write_your_message' => 'اكتب رسالتك',
    'edit' => 'تعديل',
    'select_language' => 'اختر اللغة',
    'comments_for_order_number' => 'تعليقات الطلب رقم - ',
    'statuses_for_order_number' => 'حالات الطلب رقم - ',
    'invoices_for_order_number' => 'فواتير الطلب رقم - ',
    'saved_successfully' => 'تم الحفظ بنجاج',
    'blocked_account' => 'تم تعطيل حسابك ، يرجى مراجعة المسؤول',
    'add_company' => 'اضافة شركة',
    'edit_company' => 'تعديل شركة',
    'edit_status' => 'تعديل الحالة',
    'add_account' => 'اضافة حساب',
    'edit_account' => 'تعديل حساب',
    'index' => 'الترتيب',
    'edit_settings' => 'تعديل اعدادات النظام',
    'logo' => 'الشعار',
    'favicon' => 'الشعار العلوي',
    'fax' => 'الفاكس',
    'address_ar' => 'العنوان بالعربي',
    'address_en' => 'العنوان بالانجليزي',


    'leaves' => 'الاجازات',
    'add_leave' => 'اضافة اجازة',
    'start_date' => 'تاريخ البداية',
    'end_date' => 'تاريخ النهاية',
    'edit_leave' => 'تعديل اجازة',
    'increases' => 'الزيادات',
    'add_increase' => 'اضافة زيادة',
    'increase_date' => 'تاريخ الزيادة',
    'edit_increase' => 'تعديل زيادة',
    'no_leaves_found' => 'لا يوجد اجازات',
    'no_increases_found' => 'لا يوجد زيادات',
    'leave_days_count' => 'عدد ايام الاجازة',
    'no_attachments_found' => 'لا يوجد مرفقات',
    'nationality' => 'الجنسية',
    'gender' => 'الجنس',

    'pending_leave' => 'قيد المراجعة',
    'approved' => 'معتمدة',
    'rejected' => 'مرفوضة',
    'annual' => 'سنوية',
    'unpaid' => 'غير مدفوعة',
    'sick' => 'مرضية',
    'urgent' => 'طارئة',
    'basic' => 'اساسية',
    'allowance' => 'بدلات',



    'salaryActions' => 'الزيادات والخصومات',
    'add_salaryAction' => 'اضافة زيادة / خصم',
    'edit_salaryAction' => 'تعديل زيادة / خصم',
    'increase' => 'زيادة',
    'deduction' => 'خصم',
    'salary_action_pending' => 'قيد المراجعة',
    'salary_action_done' => 'تم التنفيذ',
    'no_salaryActions_found' => 'لا يوجد زيادات او خصومات',





    // Employees
    'add_employee' => 'اضافة موظف',
    'add_employee_details' => 'اضافة بيانات موظف',
    'edit_employee_details' => 'تعديل بيانات موظف',
    'user_details' => 'بيانات المستخدم',
    'employee_details' => 'بيانات الموظف',
    'joinDate' => 'تاريخ التعيين',
    'recidencyExpirationDate' => 'تاريخ انتهاء الاقامة',
    'passportExpirationDate' => 'تاريخ انتهاء الجواز',
    'passportIssueDate' => 'تاريخ اصدار الجواز',
    'passport_no' => 'رقم جواز السفر',
    'startingSalary' => 'الراتب الابتدائي',
    'startingLeaveBalance' => 'رصيد الاجازات الابتدائي',
    'lastWorkingDate' => 'تاريخ اخر يوم دوام',
    'active_employee' => 'يداوم',
    'resigned' => 'مستقيل',
    'terminated' => 'منهي خدماته',
    'under_test' => 'تحت التجربة',
    'iban' => 'ايبان',
    'salary' => 'الراتب',
    'leave_balance' => 'رصيد الاجازات',
    'net_working_days' => 'ايام العمل',
    'indemnity' => 'مكافأة نهاية الخدمة',
    'male' => 'ذكر',
    'female' => 'انثى',


    'absences' => 'الغيابات',
    'add_absence' => 'اضافة غياب',
    'edit_absence' => 'تعديل غياب',
    'deduction_days' => 'عدد ايام الخصم',
    'deduction_amount' => 'مبلغ الخصم',
    'absence_days_count' => 'عدد ايام الغياب',
    'no_absences_found' => 'لا يوجد غيابات',


    'attachments' => 'المرفقات',
    'add_attachment' => 'اضافة مرفق',
    'description_ar' => 'الوصف (عربي)',
    'description_en' => 'الوصف (انجليزي)',
    'description' => 'الوصف',
    'select_file' => 'اختر الملف',
    'expirationDate' => 'تاريخ الانتهاء',
    'alertable' => 'تنبيه',
    'alertBefore' => 'عدد ايام التنبيه',
    'upload_done' => 'تم تحميل الملف',





    'voucher_number' => 'رقم القيد',
    'manual_id' => 'الرقم اليدوي',
    'add_journal_voucher' => 'اضافة قيد',
    'edit_journal_voucher' => 'تعديل قيد رقم ',
    'account' => 'الحساب',
    'cost_center' => 'مركز التكلفة',
    'contact' => 'الموظف المرتبط',
    'narration' => 'البيان',
    'debit' => 'مدين',
    'credit' => 'دائن',
    'copy_from' => 'نسخ من',
    'copy' => 'نسخ',
    'invalid_voucher_number' => 'لا يوجد قيد بهذا الرقم',
    'balance' => 'الرصيد',
    'opening_balance' => 'رصيد اول المدة',
    'usage' => 'الاستخدام',




    'receiver' => 'المستلم',
    'delivery' => 'التوصيل',
    'discount' => 'الخصم',
    'discount_must_be_less_than_or_equal_services' => 'يطبق الخصم على اعمال اصيانة فقط',
    'services_after_discount' => 'الخدمات بعد الخصم',






    //Sidebar
    'accounting' => 'الحسابات',
    'accounts' => 'الحسابات',
    'journal_vouchers' => 'القيود اليومية',
    'bank_payment' => 'سندات الصرف البنكية',
    'bank_receipt' => 'سندات القبض البنكية',
    'account_statement' => 'كشف حساب',
    'balance_sheet' => 'الميزانية العمومية',
    'trial_balance' => 'ميزان المراجعة',
    'profit_loss' => 'الارباح والخسائر',
    'hr' => 'شؤون الموظفين',
    'employees' => 'الموظفين',
    'fleet' => 'السيارات',
    'cars' => 'السيارات',
    'companies' => 'الشركات',
    'profile' => 'الملف الشخصي',
    'logout' => 'تسجيل خروج',
    'manage_account' => 'ادارة الحساب',
    'assets' => 'الاصول',




    //Cars
    'add_car' => 'اضافة سيارة',
    'edit_car' => 'تعديل سيارة',
    'car_code' => 'كود السيارة',
    'brand' => 'المصنع',
    'car_type' => 'الشكل',
    'driver' => 'السائق',
    'management_no' => 'الرقم الاداري',
    'plate_no' => 'رقم اللوحة',
    'insurance_expiration_date' => 'تاريخ انتهاء التأمين',
    'company' => 'الشركة',
    'passengers_no' => 'عدد الركاب',
    'adv_expiration_date' => 'تاريخ انتهاء الاعلان',
    'has_installment' => 'مطلوبة للاقساط',
    'installment_company' => 'شركة التقسيط',
    'company_data' => 'بيانات الشركة',
    'manufacture_data' => 'بيانات المصنع',
    'driver_data' => 'بيانات السائق',
    'kilos' => 'عداد الكيلو',
    'fuel' => 'عداد البنزين',
    'assign' => 'تسليم السيارة للسائق',
    'unassign' => 'استلام السيارة من السائق',
    'click_to_unassign' => 'اضغط هنا لاستلام السيارة من السائق',
    'car_actions' => 'حركات السيارة',
    'action_type' => 'نوع الحركة',
    'add_car_action' => 'تسجيل حركة',
    'car_services' => 'صيانات السيارة',
    'add_car_service' => 'تسجيل صيانة',
    'cost' => 'التكلفة',
    'expected' => 'المتوقع',
    'fuel_card_serial' => 'سيريال كارت البنزين',
    'fuel_card_number' => 'رقم كارت البنزين',




    'cashier' => 'التحصيل',
    'cash_collection' => 'تحصيل نقدي',
    'knet_collection' => 'تحصيل K-Net',
    'collect' => 'تحصيل',
    'knet_ref_number' => 'رقم التفويض',
    'add_part' => 'اضافة قطعة',
    'internal' => 'داخلي',
    'external' => 'خارجي',
    'part_source' => 'مصدر القطعة',
    'internal_parts' => 'بضاعة داخلية',
    'external_parts' => 'بضاعة خارجية',
    'no_uncollected_payments' => 'لا يوجد دفعات غير محصلة',
    'income_account_id' => 'حساب الايراد',
    'cost_account_id' => 'حساب التكاليف',
    'cost_centers' => 'مراكز التكلفة',
    'add_cost_center' => 'مركز تكلفة جديد',
    'edit_cost_center' => 'تعديل مركز التكلفة ',



    'knet_tax' => 'عمولة ال K-Net',
    'cash_account_id' => 'حساب النقدية',
    'bank_account_id' => 'حساب البنك',
    'bank_charges_account_id' => 'حساب العمولات البنكية',
    'receivables_account_id' => 'حساب المؤجلات',
    'internal_parts_account_id' => 'حساب البضاعة الداخلية',

    'suppliers' => 'الموردين',
    'supplier' => 'المورد',
    'add_supplier' => 'مورد جديد',
    'edit_supplier' => 'تعديل المورد ',
    'part_invoices' => 'فواتير بضاعة العهدة ',
    'add_part_invoice' => 'تسجيل فاتورة بضاعة عهدة',
    'cost_amount' => 'التكلفة',
    'sales_amount' => 'سعر البيع',
    'edit_part_invoice' => 'تعديل فاتورة بضاعة العهدة رقم ',



    'edit_discount' => 'تعديل الخصم ',
    'ratings' => 'التقييمات ',
    'rating' => 'التقييم ',
    'rate_order_no' => 'تقييم الطلب رقم ',
    'invoice_required' => 'الفاتورة اجبارية ',
    'total_car_services_cost' => 'اجمالي تكاليف الصيانة ',
    'uploading' => 'جاري رفع الملف ',
    'press_here_to_rate' => 'تقييم الخدمة ',
    'view' => 'عرض ',
    'send_survey' => 'ارسال استبيان',
    'orders_per_month' => 'عدد الطلبات لكل شهر',
    'completed_orders_per_month' => 'عدد الطلبات المنفذة لكل شهر',
    'max_export_size' => 'لا يمكن تصدير اكثر من :maxExportSize سجل',
    'exporting' => 'جاري التصدير .. يرجى الانتظار',
    'hidden' => 'مخفي',
    'edit_marketing' => 'تعديل تسويق',
    'wrong_voucher' => 'قيد خطأ',
    'count' => 'العدد',
    'journal_voucher' => 'قيد يومي',
    'invoice_voucher' => 'فاتورة مبيعات',
    'invoice_cash_payment_voucher' => 'دفعة مبيعات نقدية',
    'invoice_knet_payment_voucher' => 'دفعة مبيعات كي نت',
    'part_invoice_voucher' => 'فاتورة بضاعة عهدة',
    'cost_voucher' => 'قيد تكاليف',
    'hold_order' => 'ايقاف مؤقت للطلب',
    'please_provide_reason_for_hold' => 'يرجى تحديد سبب الايقاف المؤقت للطلب',
    'please_provide_reason_for_cancel' => 'يرجى تحديد سبب الغاء الطلب',
    'confirm_cancel' => 'تأكيد الالغاء',
    'confirm_hold' => 'تأكيد التوقف',
    'add_permission' => 'اضافة صلاحية',
    'section_name_ar' => 'اسم القسم بالعربي',
    'section_name_en' => 'اسم القسم بالانجليزي',
    'desc_ar' => 'الوصف بالعربي',
    'desc_en' => 'الوصف بالانجليزي',
    'edit_permission' => 'تعديل صلاحية',
    'details_for_order_number' => 'تفاصيل الطلب رقم',
    'no_marketings' => 'لا يوجد تسويق',
    'has_active_orders' => 'لديه طلبات معينه',
    'daily_statistics' => 'احصائيات يومية',
    'todays_completed_orders_count' => 'الطلبات المنفذة',
    'todays_cancelled_orders_count' => 'الطلبات الملغية',
    'todays_customers_count' => 'عملاء اليوم',
    'collected' => 'محصل',
    'uncollected' => 'غير محصل',
    'uncollect' => 'الغاء التحصيل',
    'daily_review' => 'المراجعة اليومية',
    'parts_difference' => 'فرق العهدة',
    'grand_total' => 'الاجمالي الكلي',
    'company_technicians' => 'فنيي الشركة',
    'commission_technicians' => 'فنيي النسبة',
    'net_knet' => 'صافي كي نت',
    'knet_charges' => 'عمولة كي نت',
    'knet_total' => 'احمالي كي نت',
    'cash_total' => 'اجمالي النقدية',
    'collection_statement' => 'كشف تحقيق التحصيل',
    'bank_deposit_required' => 'المبلغ المطلوب ايداعه في البنك',
    'users_receivables' => 'كشف ذمم الموظفين',
    'transactions_balance' => 'رصيد الحركة',
    'closing_balance' => 'الرصيد الختامي',
    'parts_total' => 'اجمالي بضاعة العهدة',
    'expected_invoices_deletion' => 'فواتير محتمل الغائها',
    'deleted_invoices_counter' => 'احصائية الفواتير الملغية',
    'no_deleted_invoices' => 'لا يوجد فواتير ملغية',
    'duplicate' => 'استنساخ',
    'invoices_target' => 'تارجت الفواتير',
    'amount_target' => 'تارجت الايراد',
    'targets' => 'ادخال التارجت',


    'completed_orders' => 'الطلبات المنفذة',
    'required_orders_target' => 'الطلبات المطلوبة',
    'orders_shortage' => 'عجز الطلبات',
    'delivery_total' => 'اجمال التوصيل',
    'total_income' => 'اجمالي الايراد',
    'required_services_amount_target' => 'ايراد الصيانة المطلوب',
    'services_amount_shortage' => 'عجز ايراد الصيانة',
    'done_percentage' => 'النسبة المحققة',
    'shortage_percentage' => 'نسبة العجز',
    'target_statement' => 'كشف تحقيق النسبة',
    'shift_target_statement' => 'كشف تحقيق النسبة للشفتات',
    'remaining_days' => 'عدد الايام المتبقية',
    'upcommingExpiredAttachments' => 'مرفقات تنتهي قريبا',
    'expiredAttachments' => 'مرفقات منتهية',
    'alerts' => 'تنبيهات',
    'total_technicians' => 'عدد الفنيين',
    'average_completed_orders_for_technicians' => 'متوسط التنفيذ لكل فني',
    'pending_invoices_report' => 'كشف الفواتير المؤجلة',







    'contracts' => 'العقود',
    'contract' => 'عقد',
    'construction_contracts' => 'عقود الانشائي',
    'subscription_contracts' => 'العقود السنوية',
    'add_contract' => 'اضافة عقد',
    'edit_contract' => 'تعديل عقد',
    'contract_type' => 'نوع العقد',
    'building_type' => 'نوع المبنى',
    'contract_number' => 'رقم العقد',
    'contract_date' => 'تاريخ بداية العقد',
    'contract_duration' => 'مدة العقد',
    'units_count' => 'عدد الوحدات',
    'central_count' => 'عدد السنترال',
    'contract_value' => 'قيمة العقد',
    'collected_amount' => 'اجمالي المحصل',
    'contract_expiration_date' => 'تاريخ انتهاء العقد',
    'subscription' => 'عقد سنوي',
    'construction' => 'انشائي',
    'residential' => 'سكني',
    'commercial' => 'استثماري',
    'sp_included' => 'يشمل قطع غيار',


    'quotations' => 'عروض الاسعار',
    'quotation_number' => 'رقم عرض السعر',
    'add_quotation' => 'اضافة عرض سعر',
    'edit_quotation' => 'تعديل عرض سعر',
    'sn' => 'م',









    'add_phoneDevice' => 'اضافة جهاز تليفون',
    'edit_phoneDevice' => 'تعديل جهاز تليفون',
    'model' => 'الموديل',
    'sim_no' => 'رقم الشريحة',
    'phone_devices' => 'اجهزة التليفونات',
    'phone_device_actions' => 'حركة جهاز',
    'add_phone_device_action' => 'اضافة حركة',





    'administration' => 'الشؤون الادارية',
    'company_contracts' => 'العقود والاتفاقيات',
    'company_budgets' => 'ميزانيات الشركات',
    'add_company_budget' => 'اضافة ميزانية',
    'edit_company_budget' => 'تعديل ميزانية',
    'client_name' => 'اسم العميل',
    'add_company_contract' => 'اضافة عقد / اتفاقية',
    'initiation_date' => 'تاريخ البداية',
    'expiration_date' => 'تاريخ النهاية',
    'edit_company_contract' => 'تعديل عقد / اتفاقية',




    'refresh' => 'تحديث',
    'document_types' => 'انواع السندات',
    'documents' => 'السندات',
    'add_document_type' => 'اضافة نوع سند',
    'edit_document_type' => 'تعديل نوع سند',
    'add_document' => 'اضافة سند',
    'document_number' => 'رقم السند',
    'document_serial_from' => 'مسلسل من',
    'document_serial_to' => 'مسلسل الى',
    'document_pages' => 'عدد الصفحات',
    'receiver_id' => 'المستلم',
    'receiving_date' => 'تاريخ الاستلام',
    'back_date' => 'تاريخ الارجاع',
    'expired' => 'منتهي',
    'document_type_id' => 'نوع السند',
    'edit_document' => 'تعديل سند',
    'mass_collect' => 'تحصيل الكل',
    'collecting' => 'جاري التحصيل ...',
    'invoices_per_technician_report' => 'الفواتير حسب الفنيين خلال فترة',




    'profit_loss_report' => 'تقرير الارباح والخسائر',
    'incomes' => 'الايرادات',
    'costs' => 'التكاليف',
    'expenses' => 'المصروفات',
    'total_incomes' => 'اجمالي الايرادات',
    'total_costs' => 'اجمالي التكاليف',
    'total_expenses' => 'اجمالي المصروفات',
    'profit' => 'الربح',
    'loss' => 'الخسارة',
    'summary' => 'الملخص',
    'cancel_reason' => 'سبب الالغاء',

    'letters' => 'المراسلات',
    'select_type' => 'اختر النوع',
    'incoming' => 'وارد',
    'outgoing' => 'صادر',
    'reference' => 'المرجع',
    'subject' => 'الموضوع',
    'sender' => 'المرسل',
    'receiver' => 'المستلم',
    'date' => 'التاريخ',
    'type' => 'النوع',
    'delete' => 'حذف',
    'type_required' => 'يجب اختيار نوع المراسلة',
    'sender_required' => 'يجب ادخال المرسل',
    'receiver_required' => 'يجب ادخال المستلم',
    'subject_required' => 'يجب ادخال الموضوع',
    'edit_letter' => 'تعديل المراسلة',
    'add_letter' => 'اضافة مراسلة',
    'load_more' => 'تحميل المزيد',
    'no_attachments' => 'لا يوجد مرفقات',
    'description_ar_required' => 'يجب ادخال الوصف بالعربي',
    'description_en_required' => 'يجب ادخال الوصف بالانجليزي',
    'file_required' => 'يجب ادخال الملف',
    'file_file' => 'يجب ان يكون الملف ملف',
    'file_mimes' => 'يجب ان يكون الملف من نوع pdf,doc,docx,xls,xlsx,ppt,pptx',
    'edit_attachment' => 'تعديل مرفق',
    'total_amount' => 'اجمالي المبلغ',
    'in_progress_orders' => 'طلبات قيد التنفيذ',
    'basic_info' => 'البيانات الاساسية',
    'new_comment_received' => 'تم التعليق على الطلب',
    'new_order_received' => 'تم اضافة طلب جديد',
    'todays_completed_orders_for_technician' => 'منفذ اليوم للفني',
    'filters' => 'الفلاتر',
    'show_filters' => 'عرض الفلاتر',
    'hide_filters' => 'إخفاء الفلاتر',
    'reset_filters' => 'إعادة تعيين الفلاتر',
    'customer_information' => 'معلومات العميل',
    'order_information' => 'معلومات الطلب',
    'date_filters' => 'فلاتر التاريخ',
    'orderTag' => 'وسم الطلب',
    'block' => 'القطعة',
    'street' => 'الشارع',
    'are_u_sure' => 'هل أنت متأكد؟',
    'edit_order' => 'تعديل الطلب',
    'in_progress_orders_for_customer' => 'طلبات قيد التنفيذ للعميل',
    'all_orders_for_customer' => 'طلبات العميل',
    'you_dont_have_permission_to_create_order' => 'ليس لديك صلاحية لإنشاء طلب',
    'you_dont_have_permission_to_update_order' => 'ليس لديك صلاحية لتعديل هذا الطلب',
    'you_can_only_update_department_while_the_order_is_in_created_status' => 'يمكنك فقط تعديل القسم أثناء حالة الطلب في حالة الإنشاء',
    'order_is_already_received_or_arrived_or_completed_or_cancelled' => 'الطلب مستلم أو وصل أو منفذ أو ملغي',
    'technician_is_not_active' => 'الفني غير نشط',
    'technician_is_not_in_the_same_department_as_the_order' => 'الفني غير في نفس القسم كالطلب',
    'order_is_already_completed_or_cancelled' => 'الطلب منفذ أو ملغي',
    'order_has_invoices' => 'الطلب لديه فواتير',
    'no_in_progress_orders_found' => 'لا يوجد طلبات قيد التنفيذ',
    'this_is_not_your_current_order' => 'هذا ليس طلبك الحالي',
    'order_is_not_destributed' => 'الطلب غير موزع',
    'you_dont_have_permission_to_accept_this_order' => 'ليس لديك صلاحية لقبول هذا الطلب',
    'you_dont_have_permission_to_arrive_this_order' => 'ليس لديك صلاحية لوصول هذا الطلب',
    'you_dont_have_permission_to_complete_this_order' => 'ليس لديك صلاحية لإنهاء هذا الطلب',
    'you_dont_have_permission_to_apply_discount' => 'ليس لديك صلاحية لتطبيق الخصم',
    'you_cant_apply_discount_to_old_invoices' => 'لا يمكنك تطبيق الخصم على الفواتير القديمة',
    'discount_cannot_be_negative' => 'لا يمكن أن يكون الخصم سالب',
    'invoice_has_payments_you_cant_apply_discount' => 'الفاتورة لديها دفعات، لا يمكنك تطبيق الخصم',
    'discount_cannot_be_greater_than_the_invoice_services_amount' => 'لا يمكن أن يكون الخصم أكبر من مبلغ الخدمات في الفاتورة',
    'you_dont_have_permission_to_delete_this_invoice' => 'ليس لديك صلاحية لحذف هذه الفاتورة',
    'order_has_only_one_invoice_you_cant_delete_it' => 'الطلب لديه فاتورة واحدة، لا يمكنك حذفها',
    'invoice_has_collected_payments_you_cant_delete_it' => 'الفاتورة لديها دفعات محصلة، لا يمكنك حذفها',
    'you_dont_have_permission_to_create_payment' => 'ليس لديك صلاحية لإنشاء دفعة',
    'amount_cannot_be_negative_or_zero' => 'لا يمكن أن يكون المبلغ سالب أو صفر',
    'amount_cannot_be_greater_than_the_invoice_remaining_amount' => 'لا يمكن أن يكون المبلغ أكبر من مبلغ الفاتورة المتبقي',
    'you_dont_have_permission_to_delete_this_payment' => 'ليس لديك صلاحية لحذف هذه الدفعة',
    'payment_is_already_collected_you_cant_delete_it' => 'الدفعة محصلة، لا يمكنك حذفها',
    'you_can_only_export_up_to_5000_records' => 'يمكنك فقط تصدير حتى 5000 سجل',
    'you_dont_have_permission_to_create_customer' => 'ليس لديك صلاحية لإنشاء عميل',
    'you_dont_have_permission_to_delete_this_customer' => 'ليس لديك صلاحية لحذف هذا العميل',
    'you_dont_have_permission_to_update_this_customer' => 'ليس لديك صلاحية لتعديل هذا العميل',
    'delete_customer_confirmation' => 'هل أنت متأكد من حذف هذا العميل؟',
    'customer_has_orders_you_cant_delete_it' => 'العميل لديه طلبات، لا يمكنك حذفه',
    'are_you_sure_delete_invoice' => 'هل أنت متأكد من حذف هذه الفاتورة؟',
    'you_dont_have_permission_to_view_invoices' => 'ليس لديك صلاحية لعرض الفواتير',
    'you_dont_have_permission_to_create_invoice' => 'ليس لديك صلاحية لإنشاء فاتورة',
    'are_you_sure_you_want_to_delete_this_payment' => 'هل أنت متأكد من حذف هذه الدفعة؟',
    'you_dont_have_permission_to_dispatch_orders' => 'ليس لديك صلاحية لتوزيع الطلبات',
    'you_dont_have_permission_to_cancel_orders' => 'ليس لديك صلاحية لإلغاء الطلبات',
    'you_dont_have_permission_to_hold_orders' => 'ليس لديك صلاحية لإيقاف الطلبات',
    'are_you_sure_you_want_to_hold_this_order' => 'هل أنت متأكد من إيقاف هذا الطلب مؤقتا؟',
    'technician_is_not_active' => 'الفني غير نشط',
    'order_is_already_completed_or_cancelled_or_received_or_arrived' => 'الطلب منفذ أو ملغي أو مستلم أو وصل',
    'you_dont_have_permission_to_create_customer_contract' => 'ليس لديك صلاحية لإنشاء عقد عميل',
    'this_will_set_this_order_as_first_order' => 'سيتم تعيين هذا الطلب كأول طلب للفني - هل أنت متأكد؟',
    'technician_refund' => 'مردودات - فنيين',
    'contractor_refund' => 'مردودات - مقاولين',
    'costs' => 'تكاليف تشغيل',
    'deduct_from_employee' => 'خصم على الموظف',
    'add_reconciliation' => 'اضافة تسوية',
    'reconciliations' => 'تسويات',
    'months' => [
        '1' => 'يناير',
        '2' => 'فبراير',
        '3' => 'مارس',
        '4' => 'ابريل',
        '5' => 'مايو',
        '6' => 'يونيو',
        '7' => 'يوليو',
        '8' => 'اغسطس',
        '9' => 'سبتمبر',
        '10' => 'اكتوبر',
        '11' => 'نوفمبر',
        '12' => 'ديسمبر',
    ],
    'customers_completed_orders_statistics' => 'احصائيات الطلبات المنفذة للعملاء',
    'customers_with_no_orders' => 'عملاء ليس لديهم طلبات',
    'completed_orders_count' => 'عدد الطلبات المنفذة',
    'equal' => 'يساوي',
    'greater_than' => 'اكبر من',
    'greater_than_or_equal_to' => 'اكبر من او يساوي',
    'less_than' => 'اصغر من',
    'less_than_or_equal_to' => 'اصغر من او يساوي',
    'direct_permissions' => 'الصلاحيات المباشرة',
    'you_dont_have_permission_to_change_user_status' => 'ليس لديك صلاحية لتعديل حالة الموظف',
    'user_has_in_progress_orders' => 'الموظف لديه طلبات قيد التنفيذ',
    'you_dont_have_permission_to_edit_user' => 'ليس لديك صلاحية لتعديل الموظف',
    'new_account' => 'حساب جديد',
    'cancel_survey_message_title' => 'استبيان إلغاء طلب',


    'cancel_survey_message' => [
        'line_1' => 'بناء على رغبتكم تم الغاء الطلب',
        'line_2' => 'حرصا منا على تحسين تجربتكم',
        'line_3' => 'يرجى تحديد سبب الغاء الطلب',
    ],
    'delayed_arrival' => 'تأخر في الوصول',
    'unsuitable_prices' => 'الأسعار غير مناسبة',
    'issue_self_resolved' => 'تم حل المشكلة ذاتيًا',
    'no_response_communication' => 'عدم الرد أو التواصل',
    'other_reason' => 'سبب آخر',
    'submit_feedback' => 'إرسال الاستبيان',
    'other_reason_placeholder' => 'يرجى التوضيح',
    'add_to_survey_list' => 'إضافة إلى الاستبيانات',
    'remove_from_survey_list' => 'إزالة من الاستبيانات',
    'surveys' => 'الاستبيانات',
    'survey_messages_sent_successfully' => 'تم إرسال الاستبيانات بنجاح',
    'you_do_not_have_permission_to_send_survey_messages' => 'ليس لديك صلاحية لإرسال الاستبيانات',
    'failed_to_send_survey_messages' => 'فشل إرسال الاستبيانات',
    'already_submitted_survey' => 'تم استلام استبيانكم لهذا الطلب',
    'thank_you_feedback' => 'نشكركم لحسن تعاونكم',
    'cancel_reason_required' => 'يرجى اختيار سبب الغاء الطلب',
    'cancel_surveys' => 'استبيانات الطلبات الملغية',

    'missed_call' => 'مكالمة فائتة',

    'loading' => 'جاري التحميل ...',

    'set_appointment' => 'تعيين موعد',
    'appointment_time_is_now' => 'موعد استحقاق الطلب',
    'this_will_delete_the_appointment' => 'سيتم حذف الموعد',
    'percentage_payslips' => 'رواتب فنيين النسبة',
    'survey_counter' => 'عدد الاستبيانات',
    'other_income_categories' => 'انواع الإيرادات الأخرى',
    "add_other_income_category" => "اضافة نوع إيرادات أخرى",
    "edit_other_income_category" => "تعديل نوع إيرادات أخرى",
    "expense_account_id" => "حساب المصروف",
    "knet_account_id" => "حساب الكي نت",
    "other_income_category" => "نوع الإيراد",
    "add_income_invoice" => "اضافة فاتورة إيراد",
    "edit_income_invoice" => "تعديل فاتورة إيراد",
    "income_invoices" => "فواتير ايرادات الادارات الاخرى",
    "bank_deposit" => "ايداع بنكي",
    "view_payments" => "عرض الدفعات",
    "add_income_payment" => "اضافة دفعة إيراد",
    "edit_income_payment" => "تعديل دفعة إيراد",
    "bank_account" => "حساب البنك",
    "created_by" => "انشئ بواسطة",
    "income_report" => "تقرير الإيرادات",
];
